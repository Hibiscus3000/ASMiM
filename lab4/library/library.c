...
unsigned long long rd(tested *array, long long number_of_elements, void *param)
{
    ...
    a = memchr((void *)array,1,number_of_elements * sizeof(tested));
...
unsigned long long wr(tested *array, long long number_of_elements, void *param)
{
    ...
    unsigned long long startt = __rdtscp(&AUX);
    memset((void*)array, to_write_value,number_of_elements * sizeof(tested));
    unsigned long long endt = __rdtscp(&AUX);
    return endt - startt;
}
...
unsigned long long cp(tested *array, long long number_of_elements, void *param)
{
    ...
    unsigned long long startt = __rdtscp(&AUX);
    memcpy(new_array, array, number_of_elements * sizeof(tested));
    unsigned long long endt = __rdtscp(&AUX);
    free(new_array);
    return endt - startt;
}
...
{
    char *end_ptr;
    long long arg = strtol(argv, &end_ptr, 10);
    if (end_ptr == argv)
    {
...
int main(int argc, char *argv[])
{
    ...
    long long number_of_threads = get_arg(argv[1], "number of threads");
    long long number_of_cpus = get_arg(argv[2], "number of cpus");
    long long number_of_virual_threads = get_arg(argv[3], "number of virual threads");
    long long array_size = get_arg(argv[4], "array size");
    memory_alloc mem_al = get_arg(argv[5], "memory allocation type");
    long long to_write_value = get_arg(argv[6], "to write value");
