...
struct thread_data_t
{
    ...
    int cpu;
    unsigned long long (*test)(tested *, int, void *);
    // array_size in bytes
    int array_size;
    memory_alloc mem_al;
    void *add_param;
...
void check_code(int code, const char *msg)
{
    if (code)
    {
        char buf[BUFSIZ];
...
tested *get_array(int number_of_elements, memory_alloc mem_al)
{
    switch (mem_al)
    {
    case dyn:
        return (tested *)malloc(sizeof(tested) * number_of_elements);
    case align:
        void *arr;
        int code = posix_memalign(&arr, sizeof(tested), sizeof(tested) * number_of_elements);
        check_code(code, "posix_memalign");
        if (code)
            return NULL;
        return (tested *)arr;
    }
...
unsigned long long rd(tested *array, int number_of_elements, void *param)
{
    unsigned int AUX;
    tested a;
...
unsigned long long wr(tested *array, int number_of_elements, void *param)
{
    unsigned int AUX;
    tested to_write_value = (tested)param;
    unsigned long long startt = __rdtscp(&AUX);
    for (int i = 0; i < number_of_elements; ++i)
        array[i] = to_write_value;
    unsigned long long endt = __rdtscp(&AUX);
    return endt - startt;
}

unsigned long long cp(tested *array, int number_of_elements, void *param)
{
    unsigned int AUX;
...
void *test_param(void *param)
{
    ...
    int number_of_elements = t_data->array_size / sizeof(tested);
    tested *array = get_array(number_of_elements, t_data->mem_al);
    if (!array)
    {
        fprintf(stderr, "%d: unable to allocate memory for array", getpid());
        t_data->ticks = -1;
        return NULL;
    }
...
void test(long number_of_threads, long number_of_cpus, long number_of_virtual_threads,
          long array_size, memory_alloc mem_al, long to_write_value, int results_table_fd)
{
    pthread_t threads[number_of_threads - 1];
    int number_of_cores = number_of_cpus / number_of_virtual_threads;
    int array_size_per_thread = array_size / number_of_threads;
    unsigned long long (*test[3])(tested *, int, void *) = {rd, wr, cp};
...
long get_arg(char *argv, const char *arg_name)
{
    char *end_ptr;
    int arg = strtol(argv, &end_ptr, 10);
    if (end_ptr == argv)
    {
...
int main(int argc, char *argv[])
{
    if (argc < 7)
    {
        fprintf(stderr, "not enough arguments, number of threads, number of cpus, number of virtual threads, array size, memory allocation type and to write value\n");
        return 1;
    }
